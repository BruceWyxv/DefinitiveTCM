...
% and properties for connecting to and controlling a GPIB instrument. It
...
% and passed around as often as needed.
  ...
  properties (Constant = true, GetAccess = public)
    % It was discovered through long testing that 0.01 is too short, but
    % 0.02 gives the device long enough to sort out the barrage of
    % electrical impulses it is receiving via GPIB.
    commandDelay = 0.02; % Minimum time to wait between commands
  end
  ...
  methods (Static = true, Access = private)
    ...
    % Attempts to set up a connection to a device
      ...
      try
        ...
        message = sprintf('Failed to initialize GPIB device "%s" with index "%i".\n\nWhat would you like to do?', name, address);
